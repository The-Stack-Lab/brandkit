...
function build(distDir, targetDir, config) {
  ...
  // brand data with zero extra requests or scraping. <\/script> is escaped so
  // a stray "</script>" in a config value can't break out of the block.
  if (exported.brandJson) {
    var embedded = JSON.stringify(exported.brandJson).replace(/<\/script>/gi, '<\\/script>');
