var fs = require('fs');
var path = require('path');
var exporter = require('./export');

/**
 * Build a CSS font stack from a font def. When the brand's official typeface
 * isn't web-available, an optional `fallback` web stand-in is woven in:
 *   'Family', 'Fallback', sans-serif
 * No fallback → the original 'Family', sans-serif (backward-compatible).
 */
function fontStack(font) {
  if (!font || !font.family) return 'sans-serif';
  // Strip control chars (a raw newline ends a CSS string) then escape
  // backslash + single-quote, so a config value can't break out of the quoted
  // CSS string and inject rules into the generated :root block.
  function q(s) { return String(s).replace(/[\u0000-\u001F\u007F]/g, '').replace(/[\\']/g, '\\$&'); }
  var stack = "'" + q(font.family) + "'";
  if (font.fallback) stack += ", '" + q(font.fallback) + "'";
  return stack + ', sans-serif';
}

/**
 * Strip the characters a config value would need to break out of the
 * generated :root block ( < > ), out of the :root {…} rule ( { } ), or out
 * of its own declaration to smuggle in another one ( ; ). A legitimate CSS
 * token/value never contains them, so real configs pass through unchanged;
 * an adversarial value like "}</style><script>..." or
 * "red; background-image:url(x)" is defanged. Mirrors cssVal() in
 * dist/engine.js + dist/changelog.js so dev (runtime-injected) and
 * production (build-baked) :root blocks sanitize identically.
 */
function cssValue(v) {
  // Comment delimiters go too: a value carrying an unclosed `/*` would comment
  // out the rest of the generated block. Stripping them needs a fixed point,
  // because one pass can rebuild the delimiter it just removed: "//**" loses
  // the `/*` at index 1 and the surviving `/` and `*` close up into a new one.
  //
  // The strip is deliberately not string-aware. A quote-aware scanner would
  // preserve a `/*` inside url("..."), at the cost of being the kind of
  // parser that has bugs; here a mangled URL is visible and recoverable while
  // a missed delimiter silently truncates the brand's whole :root block.
  var out = String(v == null ? '' : v).replace(/[<>{};]/g, ''), prev;
  do { prev = out; out = out.replace(/\/\*|\*\//g, ''); } while (out !== prev);
  return out;
}

/**
 * Sanitize a custom-property NAME, which needs a stricter pass than a value.
 *
 * `/*` and `*\/` survive cssValue, and a CSS parser strips comments while
 * tokenizing, so a theme key written `/**\/--bk-ui-bg` was emitted verbatim
 * and read back by the browser as `--bk-ui-bg`: the reserved name, after the
 * guard had already decided the key was not reserved. Verified in Chromium.
 * A legitimate property name contains none of these characters.
 */
function cssKey(k) {
  return String(k == null ? '' : k).replace(/[\u0000-\u0020<>{}();:,'"\\\/*\[\]]/g, '');
}

/**
 * Is this a chrome token rather than a brand token?
 *
 * `--bk-*` is a reserved namespace for the variables that style brandkit's own
 * UI: the sidebar, the in-card controls, toasts. The brand paints the content;
 * the tool around it stays neutral, so a brand's typeface never ends up setting
 * the navigation (see the CHROME vs CONTENT block in dist/styles.css).
 *
 * Config themes are written by hand and by agents reading a host codebase, and
 * either can sweep up a `--bk-` name. Dropping them here means the separation
 * is enforced by the build, not only documented. Mirrored in dist/engine.js and
 * dist/changelog.js so dev and production agree.
 */
function isChromeToken(key) {
  // Tested as the browser will read it, not as it was authored: cssKey()
  // removes the characters that made `--b;k-mono-font` and `/**\/--bk-ui-bg`
  // arrive as reserved names after passing this check.
  return /^--bk-/i.test(cssKey(key));
}

/**
 * Generate a :root CSS block from config.theme + config.fonts.
 */
function generateRootCSS(config) {
  var lines = [':root {'];
  if (config.theme) {
    var keys = Object.keys(config.theme);
    for (var i = 0; i < keys.length; i++) {
      if (isChromeToken(keys[i])) continue;
      var name = cssKey(keys[i]);
      // Whatever is left has to be a custom property. A key that sanitizes to
      // something else was never a token, and emitting it would put a junk
      // declaration in :root.
      if (name.indexOf('--') !== 0 || name.length < 3) continue;
      lines.push('  ' + name + ': ' + cssValue(config.theme[keys[i]]) + ';');
    }
  }
  if (config.fonts) {
    if (config.fonts.display) {
      lines.push('  --font-display: ' + fontStack(config.fonts.display) + ';');
    }
    if (config.fonts.body) {
      lines.push('  --font-body: ' + fontStack(config.fonts.body) + ';');
    }
  }
  lines.push('}');
  return lines.join('\n');
}

/**
 * Escape a string for safe interpolation inside a double-quoted HTML attribute.
 * A legitimate URL path never contains these chars, so real basePaths pass
 * through unchanged; a hostile value can't break out of the attribute.
 */
function escapeAttr(s) {
  return String(s).replace(/&/g, '&amp;').replace(/"/g, '&quot;')
    .replace(/</g, '&lt;').replace(/>/g, '&gt;');
}

/**
 * Normalize config.basePath into an absolute, no-trailing-slash prefix.
 * "" / undefined / "/" → "" (served at root, page-relative, unchanged).
 * "brand" or "/brand/" → "/brand".
 */
function normalizeBasePath(bp) {
  if (!bp || typeof bp !== 'string') return '';
  var s = bp.trim();
  if (!s) return '';
  // Exactly one leading slash: collapse "//brand" (a protocol-relative URL:
  // browsers read it as a cross-origin host) and "brand" alike to "/brand".
  s = '/' + s.replace(/^\/+/, '');
  s = s.replace(/\/+$/, '');
  return s;
}

/**
 * Generate a Google Fonts <link> tag from config.fonts.
 */
function generateFontsLink(config) {
  if (!config.fonts) return '';
  // Dedupe families: when display and body share a font (e.g. both Inter),
  // baking it once avoids a redundant ...&family=Inter:...&family=Inter:...
  // URL and the duplicate font CSS work it triggers.
  var seen = {};
  var families = [];
  function addFamily(f) {
    if (f && f.googleImport && !seen[f.googleImport]) {
      seen[f.googleImport] = 1;
      families.push(f.googleImport);
    }
  }
  addFamily(config.fonts.display);
  addFamily(config.fonts.body);
  if (!families.length) return '';
  // One <link> per family. Joined into a single request, one unsatisfiable
  // family (a wrong name, or a weight it does not carry) 400s the whole
  // stylesheet and BOTH fonts fall back, a guide that silently renders in the
  // system face while every label names the brand typeface.
  return families.map(function (f) {
    return '<link href="https://fonts.googleapis.com/css2?family=' + f +
      '&display=swap" rel="stylesheet">';
  }).join('\n  ');
}

/**
 * Build production-ready static files from dist/ templates + config.
 * Writes index.html and styles.css into the target directory.
 */
function build(distDir, targetDir, config) {
  // Absolute base path the guide is served from (e.g. "/brand"). Empty = root /
  // page-relative, in which case nothing below is rewritten (byte-identical to
  // the pre-basePath output). The "/" + file form (no trailing slash on basePath)
  // is what lets a framework rewrite like /brand → /brand/index.html resolve.
  var basePath = normalizeBasePath(config.basePath);
  // Build styles.css: append the config :root AFTER the stylesheet's default
  // tokens so brand overrides win the cascade (styles.css ships a baseline
  // :root; an earlier block would be overridden by it).
  var cssTemplate = fs.readFileSync(path.join(distDir, 'styles.css'), 'utf8');
  var rootCSS = generateRootCSS(config);
  var builtCSS = cssTemplate + '\n\n/* Generated by brandkit build: brand overrides */\n' + rootCSS + '\n';
  fs.writeFileSync(path.join(targetDir, 'styles.css'), builtCSS);

  // Agent-native exports: brand.json (semantic), tokens.json (W3C DTCG),
  // brand.md (LLM brief). brandJson is reused to embed the data in the page.
  var exported = exporter.writeExports(config, targetDir, 'all');

  // Build index.html: inject fonts link + title + agent-discoverable brand data
  var htmlTemplate = fs.readFileSync(path.join(distDir, 'index.html'), 'utf8');
  var fontsLink = generateFontsLink(config);
  var brandName = (config.brand && config.brand.displayName) || 'Brand';

  // Inject fonts link + a discovery <link> for the structured brand data.
  // When a basePath is set, prefix the brandkit-generated refs (stylesheet,
  // engine, brand.json) so they resolve from a non-trailing-slash URL; also
  // expose the base to the runtime engine via window.__BRANDKIT_BASE__.
  // The Google Fonts link and author-supplied logo paths are left untouched.
  // basePath is author-controlled config, so it's hardened before it lands in
  // markup, each context with its own escaping:
  //  - inline <script>: JSON.stringify + "<" → <, so a "</script>" in the
  //    value can't break out of the script element (JS-string context).
  //  - href/src attributes: HTML-attribute-escaped (basePathAttr), so a stray
  //    quote can't break out of the attribute and inject a handler.
  //  - all .replace() calls below use function replacers, so a "$" in the path
  //    isn't interpreted as a $-replacement pattern.
  var basePathLiteral = JSON.stringify(basePath).replace(/</g, '\\u003C');
  var basePathAttr = escapeAttr(basePath);
  var brandJsonHref = basePath ? basePathAttr + '/brand.json' : 'brand.json';
  var headInject = (basePath ? '<script>window.__BRANDKIT_BASE__ = ' + basePathLiteral + ';</script>\n' : '') +
    (fontsLink ? fontsLink + '\n' : '') +
    '<link rel="alternate" type="application/json" href="' + brandJsonHref + '" title="Brand data">\n';
  htmlTemplate = htmlTemplate.replace('</head>', function () { return headInject + '</head>'; });

  if (basePath) {
    // Tolerant of quote style / whitespace around "=" so a future reformat of
    // the template doesn't silently no-op and ship broken (root-relative) refs.
    htmlTemplate = htmlTemplate
      .replace(/href\s*=\s*["']styles\.css["']/, function () { return 'href="' + basePathAttr + '/styles.css"'; })
      .replace(/src\s*=\s*["']engine\.js["']/, function () { return 'src="' + basePathAttr + '/engine.js"'; })
      // engine.js sets this href from BASE when it unhides the link, so a
      // browser never follows the relative one. The markup is still read by
      // things that do not run the script (crawlers, link checkers, agents),
      // and a page that prefixes four refs and not the fifth is wrong to them.
      // Leading whitespace required, so `data-href="changelog.html"` on some
      // earlier element can never absorb the one replacement.
      .replace(/(\s)href\s*=\s*["']changelog\.html["']/, function (m, sp) { return sp + 'href="' + basePathAttr + '/changelog.html"'; });
  }

  // Set title: escape the brand name so a hostile displayName can't break out
  // of the <title> element (e.g. "</title><script>\u2026").
  var brandNameHtml = escapeAttr(brandName);
  htmlTemplate = htmlTemplate.replace(
    '<title>Brand Guide</title>',
    '<title>' + brandNameHtml + ': Brand Guide</title>'
  );

  // Embed brand.json inline so an agent that fetches the page gets structured
  // brand data with zero extra requests or scraping. Every "<" is escaped to
  // its JSON unicode form (<, which JSON.parse restores to "<"), so no
  // config value can produce a "</script" sequence and break out of the block.
  if (exported.brandJson) {
    var embedded = JSON.stringify(exported.brandJson).replace(/</g, '\\u003C');
    var embed = '<script type="application/json" id="brandkit-brand">' + embedded + '</script>\n';
    htmlTemplate = htmlTemplate.replace('</body>', embed + '</body>');
  }

  fs.writeFileSync(path.join(targetDir, 'index.html'), htmlTemplate);

  // Build the standalone changelog page the same way: inject the base script +
  // fonts link + title, and (when a basePath is set) prefix its own asset refs
  // so it resolves them from a non-trailing-slash URL. It renders from
  // config.json at runtime (like index.html), so no data is embedded here.
  var changelogTemplate = fs.readFileSync(path.join(distDir, 'changelog.html'), 'utf8');
  var changelogHead = (basePath ? '<script>window.__BRANDKIT_BASE__ = ' + basePathLiteral + ';</script>\n' : '') +
    (fontsLink ? fontsLink + '\n' : '');
  changelogTemplate = changelogTemplate.replace('</head>', function () { return changelogHead + '</head>'; });
  if (basePath) {
    changelogTemplate = changelogTemplate
      .replace(/href\s*=\s*["']styles\.css["']/, function () { return 'href="' + basePathAttr + '/styles.css"'; })
      .replace(/src\s*=\s*["']changelog\.js["']/, function () { return 'src="' + basePathAttr + '/changelog.js"'; })
      // The back link is static: nothing rewrites it at runtime. Relative, it
      // only works while the changelog is served from inside basePath.
      .replace(/(\s)href\s*=\s*["']index\.html["']/, function (m, sp) { return sp + 'href="' + basePathAttr + '/index.html"'; });
  }
  changelogTemplate = changelogTemplate.replace(
    '<title>Changelog</title>',
    '<title>' + brandNameHtml + ': Changelog</title>'
  );
  fs.writeFileSync(path.join(targetDir, 'changelog.html'), changelogTemplate);

  // Copy engine.js + changelog.js as-is
  fs.copyFileSync(
    path.join(distDir, 'engine.js'),
    path.join(targetDir, 'engine.js')
  );
  fs.copyFileSync(
    path.join(distDir, 'changelog.js'),
    path.join(targetDir, 'changelog.js')
  );

  return exported;
}

module.exports = {
  generateRootCSS: generateRootCSS,
  isChromeToken: isChromeToken,
  cssKey: cssKey,
  generateFontsLink: generateFontsLink,
  normalizeBasePath: normalizeBasePath,
  build: build
};
