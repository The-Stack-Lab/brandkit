...
function seedBrandIdentity(config, projectDir) {
  ...
  // An absent or empty name is as unseeded as the scaffold value; anything
  // else is the author's own and blocks seeding entirely.
  var currentName = config.brand.name;
  var nameIsUnseeded = currentName === undefined || currentName === null ||
                       currentName === '' || currentName === SCAFFOLD_BRAND_NAME;
  if (!nameIsUnseeded) return false; // already customized
  ...
  Object.keys(identity).forEach(function (k) {
    ...
    // An empty string normally means the author cleared the field on purpose,
    // and refilling it would be the overwrite this per-field check exists to
    // prevent. `name` is the exception: the gate above already treats an empty
    // name as unseeded, so without this the name passed the gate and was then
    // skipped here — left permanently empty while every other field seeded.
