 ...
 */
/** Normalise a hex to 6 digits so #00A and #0000AA compare equal. */
function normHex(v) {
 ...
 *
 * Three bridges, in descending confidence:
 *   1. `sourceVar` — the token a swatch came from. Written since 1.6.0.
 *   2. the name — unchanged names pair trivially.
 *   3. the colour VALUE — for a config written before `sourceVar` existed,
 *      where a renamed swatch shares no key with its replacement.
 *
 * Explicitly NOT position. That was tried and pairs prose onto the wrong
 * colour: the config's array order is not the host stylesheet's declaration
 * order, so two swatches had their names and roles swapped onto each other.
 *
 * The value bridge is used only when it is UNAMBIGUOUS — one authored candidate
 * for that colour, not yet claimed. Two swatches sharing a hex would otherwise
 * both inherit the first one's prose, which is the same confidently-wrong
 * pairing in a different disguise. Anything that cannot be bridged is reported,
 * never guessed.
 */
function mergeColors(existing, extracted, scaffold) {
  var GROUPS = ['brand', 'neutrals', 'semantic'];

  // An item is brandkit's only when the WHOLE item matches a scaffold entry.
  // Keying on names alone meant an author who called a colour "Slate", "Ink" or
  // "Coral" — ordinary names the starter happens to use — had it renamed back
  // on every run with no way to make it stick.
  var scaffoldItems = {};
  GROUPS.forEach(function (g) {
    (((scaffold || {})[g] || {}).items || []).forEach(function (item) {
      scaffoldItems[JSON.stringify({ name: item.name, role: item.role })] = true;
    });
  });
  function isScaffoldItem(item) {
    return scaffoldItems[JSON.stringify({ name: item.name, role: item.role })] === true;
  }
  ...
  var bySource = {};
  var byName = {};
  var byHex = {};   // hex -> [prior, …]; a list, so ambiguity is detectable
  priors.forEach(function (item) {
    if (item.sourceVar && !bySource[item.sourceVar]) bySource[item.sourceVar] = item;
    var nameKey = String(item.name || '').toLowerCase();
    if (nameKey && !byName[nameKey]) byName[nameKey] = item;
    var hk = normHex(item.hex);
    if (hk) (byHex[hk] = byHex[hk] || []).push(item);
  });

  var claimed = [];                 // priors actually used as a source of prose
  ...
  GROUPS.forEach(function (g) {
    ...
    out[g] = {
      ...
      items: (group.items || []).map(function (item) {
        var was = (item.sourceVar && bySource[item.sourceVar]) ||
                  byName[String(item.name || '').toLowerCase()];

        if (!was) {
          var hk = normHex(item.hex);
          var candidates = (byHex[hk] || []).filter(function (c) {
            return hasProse(c) && !isScaffoldItem(c) && claimed.indexOf(c) === -1;
          });
          if (candidates.length === 1) {
            was = candidates[0];
            hexMatches.push(was.name);
          } else if (candidates.length > 1) {
            // Several authored swatches share this colour; nothing here says
            // which one this is. Report rather than pick.
            candidates.forEach(function (c) {
              if (ambiguous.indexOf(c.name) === -1) ambiguous.push(c.name);
            });
          }
        }

        if (!was || isScaffoldItem(was)) return item;
        if (claimed.indexOf(was) === -1) claimed.push(was);
  ...
  if (hexMatches.length) out._hexMatches = hexMatches;

  // Authored prose that reached nothing. Computed from what was actually
  // claimed, by ANY bridge — testing only the hex list reported a false loss
  // for every item that had matched by name, which is a false claim in the
  // release built to stop false claims.
  var lost = [];
  priors.forEach(function (item) {
    if (isScaffoldItem(item) || !hasProse(item)) return;
    if (claimed.indexOf(item) !== -1) return;
    var label = isAuthored(item.name) ? item.name : (item.hex || '(unnamed)');
    if (lost.indexOf(label) === -1) lost.push(label);
  });
