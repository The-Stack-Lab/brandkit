...
function seedBrandIdentity(config, projectDir) {
  ...
  Object.keys(identity).forEach(function (k) {
    ...
    // An empty string is only "untouched" when the scaffold itself is empty —
    // otherwise the author deliberately cleared the field and refilling it
    // would be the same overwrite this per-field check exists to prevent.
