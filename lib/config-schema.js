...
function mergeColors(existing, extracted, scaffold) {
  ...
  targets.forEach(function (t) {
    if (t.was || !t.item.sourceVar) return;
    for (var i = 0; i < available.length; i++) {
      if (available[i].sourceVar === t.item.sourceVar) { t.was = available[i]; take(available[i]); return; }
    }
  });

  // Pass 2 — name, and only when exactly one unclaimed prior carries it. Two
  // priors sharing a name are no better than none.
  targets.forEach(function (t) {
    if (t.was) return;
    var key = String(t.item.name || '').toLowerCase();
    if (!key) return;
    var hits = available.filter(function (p) {
      return String(p.name || '').toLowerCase() === key;
    });
    if (hits.length === 1) { t.was = hits[0]; take(hits[0]); }
  });

  // Pass 3 — colour value, same uniqueness rule.
  var ambiguous = [];
  targets.forEach(function (t) {
    if (t.was) return;
    var hk = normHex(t.item.hex);
    if (!hk) return;
    var hits = available.filter(function (p) { return normHex(p.hex) === hk; });
    if (hits.length === 1) { t.was = hits[0]; take(hits[0]); }
  ...
  targets.forEach(function (t) {
    ...
    if (!t.item.sourceVar || t.was.sourceVar !== t.item.sourceVar) {
