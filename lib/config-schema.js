...
function mergeColors(existing, extracted, scaffold) {
  ...
  // "Coral" — ordinary brand colour names the starter happens to use — had it
  // renamed back on every run with no way to make it stick.
  var scaffoldItems = {};
  ...
  GROUPS.forEach(function (g) {
    ...
    items.forEach(function (item) {
      ...
      // `sourceVar` was first written in 1.6.0, so every config produced by an
      // earlier version has none. Without this fallback the first 1.6.0 run of
      // an existing guide found no prior item and reverted every author-renamed
      // swatch to its generic extracted name — the exact data loss the per-item
      // merge was added to prevent.
      var nameKey = String(item.name || '').toLowerCase();
      if (nameKey && !byName[nameKey]) byName[nameKey] = item;
      var hk = String(item.hex || '').toUpperCase();
      if (hk && !byHex[hk]) byHex[hk] = item;
    });
  });

  // Prose worth carrying forward. `description` is in the documented contract
  ...
  GROUPS.forEach(function (g) {
    ...
    out[g] = {
      ...
      items: (group.items || []).map(function (item, idx) {
        ...
        // Bridge for a config written before 1.6.0, where nothing carries
        // `sourceVar` and a renamed swatch shares no key with its replacement.
        //
        // NOT position. Position was tried and is wrong: the array's order is
        // not the host stylesheet's declaration order, so two swatches had
        // their names AND roles swapped onto each other — and the viz-token
        // skip changed the extracted count, disabling the length guard exactly
        // on shadcn projects, which ship --chart-1..5.
        //
        // The colour value is the honest bridge: it identifies the same swatch
        // regardless of order, count, or what it was renamed to. If the value
        // also changed we cannot bridge, and we say so rather than guess.
        if (!was) {
          var hexKey = String(item.hex || '').toUpperCase();
          if (hexKey && byHex[hexKey]) {
            was = byHex[hexKey];
            if (isAuthored(was.name) && !isScaffoldItem(was)) hexMatches.push(was.name);
          } else if (hexKey) {
            unbridged.push(item.name);
          }
        }
  ...
  // Prior items that carried authored prose and could not be matched to
  // anything extracted. Reported so the loss is visible rather than silent.
  var lost = [];
  Object.keys(byHex).forEach(function (hk) {
    var it = byHex[hk];
    if (!isScaffoldItem(it) && (isAuthored(it.name) || isAuthored(it.role) || isAuthored(it.description)) &&
        hexMatches.indexOf(it.name) === -1 && !it.sourceVar) {
      if (lost.indexOf(it.name) === -1) lost.push(it.name);
    }
  });
  ...
  for (var i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (name === SCAFFOLD_BRAND_NAME ||
            name.replace(/^@[^/]+\//, '') === SCAFFOLD_BRAND_NAME) return true;
