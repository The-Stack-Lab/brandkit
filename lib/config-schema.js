...
var MACHINE_KEYS = {
  ...
  googleImport: true, fallback: true, url: true, 'class': true, font: true, position: true,
...
function isAuthored(v, scaffoldValue) {
  if (typeof v !== 'string') return false;
  var t = v.trim();
  if (t === '' || t.indexOf('__TODO') === 0) return false;
  if (scaffoldValue !== undefined && copyKey(v) === copyKey(scaffoldValue)) return false;
  return true;
}
...
function mergeFonts(existing, extracted, scaffold) {
  ...
  Object.keys(extracted).forEach(function (slot) {
    ...
    ['description', 'googleImport', 'fallback'].forEach(function (k) {
      if (isAuthored(was[k], scaffoldSlot[k])) merged[k] = was[k];
    });
    out[slot] = merged;
  });
