...
function mergeColors(existing, extracted, scaffold) {
  ...
  // Names/roles the starter shipped are brandkit's, not the author's.
  var scaffoldNames = {};
  GROUPS.forEach(function (g) {
    (((scaffold || {})[g] || {}).items || []).forEach(function (item) {
      if (item.name) scaffoldNames[item.name] = item.role;
    });
  });
  var prior = {};
  GROUPS.forEach(function (g) {
  ...
  GROUPS.forEach(function (g) {
    ...
    out[g] = {
      ...
      items: (group.items || []).map(function (item) {
        ...
        var isScaffoldName = Object.prototype.hasOwnProperty.call(scaffoldNames, was.name);
        if (isAuthored(was.name) && !isScaffoldName) merged.name = was.name;
        if (isAuthored(was.role, isScaffoldName ? scaffoldNames[was.name] : undefined)) {
          merged.role = was.role;
        }
        return merged;
      })
    };
