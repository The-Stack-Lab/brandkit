...
function readHostPackageName(projectDir) {
  ...
  } catch (_) { /* no package.json, or unreadable — nothing to seed from */ }
  return null;
}
...
function seedBrandIdentity(config, projectDir) {
  ...
  if (config.brand.name !== SCAFFOLD_BRAND_NAME) return false; // already customized
  var identity = hostBrandIdentity(projectDir);
  if (!identity) return false;
  Object.keys(identity).forEach(function (k) { config.brand[k] = identity[k]; });
  return true;
}
