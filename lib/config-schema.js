...
function mergeColors(existing, extracted, scaffold) {
  ...
  GROUPS.forEach(function (g) {
    ...
    out[g] = {
      ...
      items: (group.items || []).map(function (item, idx) {
        ...
        // Last resort for a config written before 1.6.0, where no item carries
        // `sourceVar` and a renamed swatch shares no key with its replacement.
        // Position is only trusted when the group's length is unchanged — then
        // the lists are the same palette in the same declaration order, which
        // makes it a strong signal rather than a guess. If the counts differ we
        // do NOT pair by position: landing one colour's name on another is
        // exactly the confidently-wrong output this release removes.
        if (!was) {
          var priorItems = (existing && existing[g] && existing[g].items) || [];
          var noProvenance = priorItems.length && priorItems.every(function (pi) { return !pi.sourceVar; });
          if (noProvenance && priorItems.length === (group.items || []).length) {
            was = priorItems[idx];
            if (was && isAuthored(was.name) && !isScaffoldItem(was)) positionalMatches.push(was.name);
          }
        }
  ...
  if (positionalMatches.length) out._positionalMatches = positionalMatches;
  return out;
}
