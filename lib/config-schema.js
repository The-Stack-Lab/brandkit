...
/**
 * Human prose worth keeping: present, not an unfilled marker, and not a value
 * brandkit itself scaffolded.
 *
 * The scaffold check is load-bearing. Without it the merge preserved the
 * starter's own `googleImport` ("Space+Grotesk:wght@…") next to a freshly
 * extracted family ("Nexa-bold"): a guide that loads one typeface while every
 * label names another. brandkit's content is not the author's work, so it does
 * not get the author's protection.
 */
/**
 * Compare copy, not bytes.
 *
 * 1.7.0 removed the em dashes from brandkit's own starter copy. Every scaffold
 * check compared strings exactly, so a guide scaffolded by 1.6.0 or earlier
 * ("Primary lockup \u2014 use on light backgrounds.") stopped matching the new
 * starter ("Primary lockup. Use on light backgrounds.") and brandkit's voice,
 * tagline and marks would have read as the client's own writing again, which
 * is the exact failure provenance exists to prevent. Punctuation and case are
 * ignored, words are not: a reworded sentence is still authored.
 */
// Only prose is compared loosely. Paths, hex values, CSS values and variable
// names are machine values: `logos/Mark.svg` is not `logos/mark.svg`, and a
// leading of "1:7" is not "1.7". Those compare exactly, and so does anything
// nested under one (`variants.svg`). Named by exclusion, because the callers
// pass whole fields (`sections`, `voice`) whose own keys are arbitrary.
var MACHINE_KEYS = {
  hex: true, oklch: true, cssVar: true, sourceVar: true, colorVar: true, css: true, value: true,
  prop: true, variants: true, size: true, tracking: true, leading: true, weight: true,
  family: true, googleImport: true, fallback: true, url: true, 'class': true, font: true, position: true,
  color: true, px: true, token: true, id: true, fg: true, bg: true, ratio: true, rating: true,
  headerLogo: true, sidebarLogo: true, version: true, date: true, sizes: true
};

function copyNormText(s) {
  return s.replace(/[\u2014\u2013,.:;()]/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
}

function copyNorm(v, machine) {
  if (typeof v === 'string') return machine ? v : copyNormText(v);
  if (Array.isArray(v)) return v.map(function (x) { return copyNorm(x, machine); });
  if (v && typeof v === 'object') {
    // Null prototype: a parsed config can carry an own `__proto__` key, and on a
    // plain object that assignment would set the prototype and hide the content
    // from the comparison, so authored work could read as scaffold.
    var out = Object.create(null);
    Object.keys(v).forEach(function (k) {
      out[k] = copyNorm(v[k], machine === true || MACHINE_KEYS[k] === true);
    });
    return out;
  }
  return v;
...
/** Stable comparison key for a value, insensitive to punctuation and case. */
function copyKey(v) {
  return JSON.stringify(copyNorm(v));
}

function isAuthored(v, scaffoldValue, key) {
  if (typeof v !== 'string') return false;
  var t = v.trim();
  if (t === '' || t.indexOf('__TODO') === 0) return false;
  // A machine value (a font import, a fallback family) is compared exactly.
  var same = MACHINE_KEYS[key] === true ? v === scaffoldValue : copyKey(v) === copyKey(scaffoldValue);
  if (scaffoldValue !== undefined && same) return false;
  return true;
}
 ...
 */
/**
 * Normalise a hex for comparison, or '' if it is not one we will compare on.
 *
 * Exactly 3 or 6 digits. Not 4 or 8: an alpha hex is a DIFFERENT colour, and
 * folding #FF000080 onto #FF0000 would hand a half-transparent red's prose to
 * an opaque one. Not 5 or 7 either, a `{3,8}` test let a typo'd `#1234567`
 * truncate to `#123456` and bridge onto a genuinely different swatch, which is
 * a guess about a value we cannot determine.
 */
function normHex(v) {
...
/**
 * Merge extracted colours over existing ones, preserving human prose.
 *
 * Resolved in GLOBAL PASSES, strongest evidence first, not per item as it
 * goes. A greedy single pass got three things wrong at once: one prior could
 * supply prose to two swatches (claimed by hex, then again by name); ambiguity
 * was decided before later, stronger evidence arrived, so a swatch was reported
 * unmatched after in fact matching; and a duplicate name beat a colour value
 * that identified its swatch uniquely.
 *
 *   1. `sourceVar`: the token a swatch came from. Written since 1.6.0.
 *   2. name: an unchanged name pairs trivially.
 *   3. colour value: for a config predating `sourceVar`, where a renamed
 *      swatch shares no other key. Used only when it identifies exactly one
 *      still-unclaimed prior.
 *
 * Explicitly NOT position: the config's array order is not the host
 * stylesheet's declaration order, so pairing by index swapped two swatches'
 * names and roles onto each other. Anything unresolvable is reported, never
 * guessed.
 */
function mergeColors(existing, extracted, scaffold) {
  var GROUPS = ['brand', 'neutrals', 'semantic'];

  // An item is brandkit's only when name, role AND description all match a
  // scaffold entry. Comparing name+role alone hid an authored description on an
  // otherwise-scaffold-shaped item, so that writing could vanish without
  // appearing in any loss report.
  var scaffoldSigs = {};
  function sig(item) {
    return copyKey({ name: item.name, role: item.role, description: item.description });
  }
  GROUPS.forEach(function (g) {
    (((scaffold || {})[g] || {}).items || []).forEach(function (item) {
      scaffoldSigs[sig(item)] = true;
    });
  });
  function isScaffoldItem(item) { return scaffoldSigs[sig(item)] === true; }
  function hasProse(item) {
    return isAuthored(item.name) || isAuthored(item.role) || isAuthored(item.description);
  }

  // Flatten both sides once; `slot` lets us write the result back in place.
  var priors = [];
  GROUPS.forEach(function (g) {
    ((existing && existing[g] && existing[g].items) || []).forEach(function (item) {
      priors.push(item);
    });
  });
  var targets = [];
  GROUPS.forEach(function (g) {
    (((extracted[g] || {}).items) || []).forEach(function (item, i) {
      targets.push({ group: g, index: i, item: item, was: null });
    });
  });

  // Only a prior carrying prose worth rescuing takes part.
  var available = priors.filter(function (p) {
    return hasProse(p) && !isScaffoldItem(p);
  });
  function take(prior) {
    var i = available.indexOf(prior);
    if (i !== -1) available.splice(i, 1);
  }

  /**
   * One pass. A key may claim a prior only when it identifies exactly one
   * available prior AND exactly one unmatched target.
   *
   * Uniqueness on BOTH sides is the point. Checking only the prior side let two
   * targets sharing a name hand the prior to whichever came first, and the
   * second target then lost the colour match that would have identified it
   * exactly: prose attached to the wrong swatch. Checking neither (pass 1
   * originally) let a duplicated sourceVar claim a prior by array order.
   */
  function claimPass(keyOf, via) {
    // Null-prototype: these are keyed on values from a client's stylesheet, so
    // a colour named `__proto__` or `constructor` reached an inherited member:
    // `.push` on Object.prototype threw and aborted the whole merge, and
    // `constructor` yielded a bogus length-1 "match".
    var priorsByKey = Object.create(null);
    available.forEach(function (p) {
      var k = keyOf(p);
      if (!k) return;
      (priorsByKey[k] = priorsByKey[k] || []).push(p);
    });
    var targetsByKey = Object.create(null);
    targets.forEach(function (t) {
      if (t.was) return;
      var k = keyOf(t.item);
      if (!k) return;
      (targetsByKey[k] = targetsByKey[k] || []).push(t);
    });
    var claimedHere = [];
    Object.keys(targetsByKey).forEach(function (k) {
      var ps = priorsByKey[k] || [];
      var ts = targetsByKey[k];
      if (ps.length !== 1 || ts.length !== 1) return; // indeterminate, leave it
      ts[0].was = ps[0];
      ts[0].via = via;
      claimedHere.push(ps[0]);
    });
    claimedHere.forEach(take);
  }

  claimPass(function (x) { return x.sourceVar || ''; }, 'sourceVar');
  claimPass(function (x) { return String(x.name || '').toLowerCase(); }, 'name');
  claimPass(function (x) { return normHex(x.hex); }, 'value');

  // Whatever colour is now shared by more than one still-unclaimed prior could
  // not be told apart, and is reported rather than resolved.
  var ambiguous = [];
  var leftoverByHex = Object.create(null);
  available.forEach(function (p) {
    var hk = normHex(p.hex);
    if (hk) (leftoverByHex[hk] = leftoverByHex[hk] || []).push(p);
  });
  var unmatchedByHex = Object.create(null);
  targets.forEach(function (t) {
    if (t.was) return;
    var hk = normHex(t.item.hex);
    if (hk) (unmatchedByHex[hk] = unmatchedByHex[hk] || []).push(t);
  });
  // Indeterminate in EITHER direction: several priors share the colour, or
  // several unmatched targets do. Reporting only the first told the user a
  // colour was "not found" when in truth it could not be told apart, the wrong
  // cause to hand someone deciding whether to re-approve a brand.
  Object.keys(unmatchedByHex).forEach(function (hk) {
    var hits = leftoverByHex[hk] || [];
    if (!hits.length) return;
    if (hits.length > 1 || unmatchedByHex[hk].length > 1) {
      hits.forEach(function (p) {
        var label = isAuthored(p.name) ? p.name : (p.hex || '(unnamed)');
        if (ambiguous.indexOf(label) === -1) ambiguous.push(label);
      });
    }
  });

  var PROSE = ['name', 'role', 'description'];
  var hexMatches = [];
  var out = {};
  GROUPS.forEach(function (g) {
    var group = extracted[g] || { label: g, items: [] };
    out[g] = { label: group.label, items: (group.items || []).slice() };
  });
  targets.forEach(function (t) {
    if (!t.was) return;
    var merged = {};
    Object.keys(t.item).forEach(function (k) { merged[k] = t.item[k]; });
    // Machine values come from this run; the writing survives it.
    PROSE.forEach(function (k) {
      if (isAuthored(t.was[k])) merged[k] = t.was[k];
    });
    out[t.group].items[t.index] = merged;
    // Only a pass-3 match was carried by colour value. Inferring it from
    // differing sourceVars labelled every name match, i.e. essentially every
    // swatch in any pre-1.6.0 upgrade: as value-bridged, a fabricated claim
    // about how the pairing was determined.
    if (t.via === 'value') {
      var lbl = isAuthored(t.was.name) ? t.was.name : t.was.hex;
      if (lbl && hexMatches.indexOf(lbl) === -1) hexMatches.push(lbl);
    }
  });

  // Whatever is still in `available` reached nothing. Reported from the real
  // end state, so a swatch that matched on a later pass is never named as lost.
  var lost = available.map(function (p) {
    return isAuthored(p.name) ? p.name : (p.hex || '(unnamed)');
  }).filter(function (v, i, a) { return a.indexOf(v) === i; });

  // Ambiguity that a later pass resolved is not ambiguity.
  ambiguous = ambiguous.filter(function (label) { return lost.indexOf(label) !== -1; });

  if (hexMatches.length) out._carriedByValue = hexMatches;
  if (lost.length) out._unmatchedAuthored = lost;
  if (ambiguous.length) out._ambiguousColours = ambiguous;
  return out;
}

/**
 * Merge extracted logos over existing ones, keyed on the asset path.
 *
 * `logos` is applied wholesale, so a filesystem rescan replaced authored logo
 * prose ("Barone Steel primary wordmark", "Minimum width 140px") with the
 * generated name and an empty description. The file path is the stable
 * identity: the same asset keeps its writing across a rescan.
 */
function mergeLogos(existing, extracted) {
  if (!Array.isArray(extracted)) return extracted;
  // Null-prototype: keyed on asset paths from the host filesystem. On a bare
  // {} a path of `__proto__` made `prior[path]` truthy via inheritance, so the
  // entry was never recorded and the authored prose was silently dropped, the
  // same defect as the colour buckets, failing quietly instead of loudly.
  var prior = Object.create(null);
  (existing || []).forEach(function (l) {
    var variants = l.variants || {};
    Object.keys(variants).forEach(function (fmt) {
      if (variants[fmt] && !prior[variants[fmt]]) prior[variants[fmt]] = l;
    });
  });
  return extracted.map(function (l) {
    var variants = l.variants || {};
    var was = null;
    Object.keys(variants).some(function (fmt) {
      if (prior[variants[fmt]]) { was = prior[variants[fmt]]; return true; }
      return false;
    });
    if (!was) return l;
    var merged = {};
    Object.keys(l).forEach(function (k) { merged[k] = l[k]; });
    ['name', 'description', 'background'].forEach(function (k) {
...
function mergeConfigs(existing, extracted) {
  ...
  var extractable = ['theme', 'spacing', 'logoSizes'];
  ...
  // Fields `generate` DERIVES from the theme, or deliberately clears because
  // they still held brandkit's own scaffold. Applied whenever present, an
  // empty value here is a decision ("these are not this brand's logos"), not an
  // absence, so the empty-array skip above must not swallow it. Without this,
  // generate computed the replacements and mergeConfigs threw them away: the
  // CLI reported removing brandkit's gradient and voice while the written
  // config kept both.
  // Logo prose is merged onto the extraction FIRST, keyed on the asset path, so
  // the value the loop below applies already carries the author's writing. A
  // filesystem rescan otherwise replaced "Barone Steel primary wordmark" /
  // "Minimum width 140px" with the generated name and an empty description.
  // A non-empty extraction only: an explicit `[]` from provenance stripping is
  // a decision and must pass through untouched.
  if (Array.isArray(extracted.logos) && extracted.logos.length) {
    extracted = Object.assign({}, extracted, {
      logos: mergeLogos(result.logos, extracted.logos)
    });
  }

  var derivable = ['gradients', 'gradientUsage', 'hierarchy', 'cssVariables',
                   'typography', 'accessibility', 'voice', 'sections',
                   'components', 'logos'];
  for (var d = 0; d < derivable.length; d++) {
    var dk = derivable[d];
    if (Object.prototype.hasOwnProperty.call(extracted, dk) &&
        extracted[dk] !== undefined && extracted[dk] !== null) {
      result[dk] = extracted[dk];
    }
  }
  ...
  var scaffold = starterConfig();
  if (extracted.colors) result.colors = mergeColors(result.colors, extracted.colors, scaffold.colors);
  if (extracted.fonts) result.fonts = mergeFonts(result.fonts, extracted.fonts, scaffold.fonts);
...
/**
 * Is `projectDir` brandkit's own repository?
 *
 * Provenance stripping must fail CLOSED. Keying it on
 * `hostBrandIdentity() !== null` failed OPEN: a project with no package.json,
 * no `name` field, or an unreadable one produced `null`, the whole mechanism
 * no-opped, and brandkit's voice and tagline shipped as the client's, with
 * `build` reporting "Ready to deploy". A plain-HTML site with a
 * tailwind.config.js is an entirely ordinary shape for that to happen in.
 *
 * So detect brandkit POSITIVELY and treat everything else as somebody else's
 * brand. Identified by its published name, or by carrying the CLI itself.
 */
function isBrandkitRepo(projectDir) {
  var dir = projectDir;
  try { dir = fs.realpathSync(path.resolve(projectDir)); } catch (_) { dir = path.resolve(projectDir); }
  ...
  for (var i = 0; i < 8; i++) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        // Exact names only. Stripping the scope before comparing meant any
        // client package called `@their-org/brandkit` identified AS brandkit,
        // silently disabling every provenance check for that project.
        if (name === SCAFFOLD_BRAND_NAME || name === '@stacklist-app/brandkit') return true;
      } catch (_) { /* unreadable, not identifying as brandkit */ }
      // The nearest manifest is the authority; a parent's name says nothing.
      break;
    }
  ...
  // Fallback for a checkout with no readable manifest: brandkit is the only
  // project that ships its own engine next to its own schema.
  return fs.existsSync(path.join(path.resolve(projectDir), 'lib', 'config-schema.js')) &&
         fs.existsSync(path.join(path.resolve(projectDir), 'bin', 'brandkit.js'));
}
...
function seedBrandIdentity(config, projectDir) {
  ...
  Object.keys(identity).forEach(function (k) {
    ...
    var untouched = current === undefined || current === null ||
                    copyKey(current) === copyKey(scaffold[k]);
    if (!untouched) return;
