...
/**
 * A brandkit guide directory ships brandkit's own placeholder marks. Scanning
 * it reads them back as the host's logos — and `init public/brand` (the natural
 * Next.js layout) puts them squarely inside the tree extract-logos walks, so
 * the next generate lists brandkit-wordmark-dark.svg as a client asset.
 * extract-css.js has guarded against exactly this since 1.4.0.
 */
...
function scanForLogos(dir, patterns, extensions, results, projectDir, depth) {
  if (depth > 3) return;
  if (isGuideDir(dir)) return; // brandkit's own marks are not the host's logos
  var entries;
  try {
