 ...
 * below. Offsets need not be preserved — callers only read declarations out
 * of the result, never positions.
 */
function stripComments(css) {
  ...
  while (i < n) {
    ...
    if (quote) {
      out += c;
      if (c === '\\' && i + 1 < n) { out += css.charAt(i + 1); i += 2; continue; }
...
function firstFamily(value) {
  ...
  if (varMatch) {
    ...
    return { family: titleCaseFamily(varMatch[1]), inferred: true };
  }
  ...
  return { family: head, inferred: false };
}
...
function fontsFromVars(cssVars) {
  ...
  if (display) out.display = { family: display.family, inferred: display.inferred };
  if (body) out.body = { family: body.family, inferred: body.inferred };
