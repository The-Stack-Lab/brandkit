...
function expand(value, vars, chain, depth) {
  ...
  while (guard++ < 200) {
    ...
    if (resolved === null && f.fallback !== undefined) {
      resolved = expand(f.fallback, vars, chain, depth + 1);
    }
    if (resolved === null) {
...
function firstFamily(value) {
  ...
  if (varMatch) {
    return titleCaseFamily(varMatch[1]);
  }
...
function pickFont(cssVars, names) {
  for (var i = 0; i < names.length; i++) {
    if (hasOwn(cssVars, names[i])) {
      var family = firstFamily(cssVars[names[i]]);
...
function fontsFromVars(cssVars) {
  ...
  if (display) out.display = { family: display };
  if (body) out.body = { family: body };
