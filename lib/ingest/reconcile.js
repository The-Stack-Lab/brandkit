...
function resolveAccent(result, pixels, sources, imageFiles) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    ...
    // that paints little area. Measure it directly across every page.
    ...
    var share = sampled ? hits / sampled : c.share;

    // Confined to a single page at trace coverage: photographic, not brand.
    if (imageFiles.length && (share < ACCENT_MIN_SHARE || onPages === 0)) {
      result.rejected.push({
        field: 'colors.primary', value: c.hex, source: 'pixel',
        reason: 'saturated but not used as a brand color — ' + pct(share) +
          ' of sampled pixels, on ' + onPages + '/' + total + ' pages'
      });
      continue;
    }
    best = { hex: c.hex, share: share, onPages: onPages };
    break;
  }
