...
function generateA11yPairs(colors) {
  ...
  for (var i = 0; i < colors.length; i++) {
    var c = colors[i];
    if (c.hex && testColors.indexOf(c.hex) === -1) {
...
function buildHierarchyFromTheme(theme) {
  if (!theme) return [];
  var fg = theme['--ink'] || theme['--foreground'] || '#111827';
  var secondary = theme['--graphite'] || theme['--slate'] || '#374151';
  var tertiary = theme['--slate'] || theme['--haze'] || '#6B7280';
  var accent = theme['--purple'] || theme['--coral'] || '#6366F1';

  // Find names from theme keys
  function findName(value) {
    var keys = Object.keys(theme);
    for (var i = 0; i < keys.length; i++) {
      if (theme[keys[i]] === value && keys[i].indexOf('rgb') === -1) {
        return keys[i].replace(/^--/, '').replace(/-/g, ' ').replace(/\b\w/g, function (c) { return c.toUpperCase(); });
      }
    }
    return value;
  }
  ...
  return [
    { class: 'h-primary', label: 'Primary', colorVar: '--ink', colorName: findName(fg), hex: fg, description: 'Primary text — body copy, headings, and content where readability is critical.' },
    { class: 'h-secondary', label: 'Secondary', colorVar: '--graphite', colorName: findName(secondary), hex: secondary, description: 'Secondary text — descriptions, supporting information, and labels.' },
    { class: 'h-tertiary', label: 'Tertiary', colorVar: '--slate', colorName: findName(tertiary), hex: tertiary, description: 'Tertiary text — captions, timestamps, footnotes, and metadata.' },
    { class: 'h-accent', label: 'Accent', colorVar: '--purple', colorName: findName(accent), hex: accent, description: 'Accent text — links, interactive labels, and calls to action.' }
  ];
}
