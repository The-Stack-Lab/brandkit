...
/**
 * Parse hex color to {r, g, b} (0 to 255).
 */
...
/**
 * sRGB channel (0 to 255) → linear light (0 to 1).
 */
...
/**
 * Linear light (0 to 1) → sRGB (0 to 255).
 */
 ...
 */
/**
 * Contrast pairs for the accessibility table.
 *
 * Two defects this replaces, both of which made the table actively misleading:
 *
 * 1. Pairs rated `Fail` were filtered out, so the table could only ever contain
 *    passes. A client read an all-pass grid no matter how the palette actually
 *    behaved, and a contrast table whose purpose is to surface problems could
 *    not, structurally, report one.
 * 2. Every pair was computed against `#FFFFFF`. For a dark-surface brand that
 *    documents a background the brand never uses.
 *
 * Failures are now kept and labelled. `opts.surfaces` supplies the brand's real
 * backgrounds; a pair that cannot be measured is omitted rather than guessed.
 */
function generateA11yPairs(colors, opts) {
  opts = opts || {};
  // No fallback to #FFFFFF. Assuming white fabricated rows against a
  // background the brand never uses: a dark-surface brand was shown
  // "Text on White 1.1:1 Fail", a failure that does not exist in its own
  // design. The comment above is the whole point: if we cannot name the
  // brand's surfaces, we have nothing to measure against and say so.
  var surfaces = (opts.surfaces || []).filter(function (s) { return s && toRgb(s.hex); });
  ...
  for (var i = 0; i < colors.length; i++) {
    var c = colors[i];
    var parsed = c.hex ? parseCssColor(c.hex) : null;
    if (!parsed || seenHexes[parsed.hex]) continue;
    seenHexes[parsed.hex] = true;
    testColors.push({ hex: parsed.hex, name: c.name || parsed.hex, group: c.group || null });
  }

  // A caution is only useful for a pair someone would actually set as text.
  // Brand and semantic colours are used for type and accents; a neutral tint
  // sitting on another neutral is a surface-on-surface pairing, and warning
  // "do not use this pair for text" about two background tones is noise that
  // buries the real failures.
  function isTextCandidate(tc, surfaceHex) {
    if (tc.group === 'brand' || tc.group === 'semantic') return true;
    // A neutral still counts when it is markedly darker than the surface:
    // that is a body/caption colour, not a background.
    var l1 = relativeLuminance(tc.hex);
    var l2 = relativeLuminance(surfaceHex);
    if (l1 === null || l2 === null) return false;
    // Absolute difference, not signed. `l2 - l1 > 0.25` only recognised DARK
    // text on a LIGHT surface, so light body text on a dark brand surface was
    // never treated as type: and its contrast failures were filtered out of
    // the cautions entirely, on exactly the brands that most need them.
    return Math.abs(l2 - l1) > 0.25;
  }

  var seenPairs = {};
  function add(fg, bg, fgName, bgName, border, textPairing) {
    // A colour that is both a surface and a swatch is reached twice, once by
    // the surface loop and once by the reverse-pair branch, putting the
    // identical row in the table twice and inflating the reported count.
    var pairKey = fg + '|' + bg;
    if (seenPairs[pairKey]) return;
    seenPairs[pairKey] = true;
    var ratio = contrastRatio(fg, bg);
    var rating = rateContrast(ratio);
    // Unmeasurable is not the same as failing: report nothing rather than a
    // rating nobody computed.
    if (!rating) return;
    pairs.push({
      fg: fg, bg: bg, fgName: fgName, bgName: bgName,
      ratio: ratio, rating: rating,
      border: !!border,
      largeText: rating === 'AA Large',
      passes: rating !== 'Fail',
      textPairing: !!textPairing
    });
  }

  for (var s = 0; s < surfaces.length; s++) {
    var surface = parseCssColor(surfaces[s].hex);
    if (!surface) continue;
    var surfaceName = surfaces[s].name || surface.hex;
    var surfaceIsLight = isLightColor(surface.hex);

    for (var j = 0; j < testColors.length; j++) {
      var tc = testColors[j];
      if (tc.hex === surface.hex) continue;
      // The colour used as text on this surface, including when it fails,
      // which is the row a designer most needs to see.
      add(tc.hex, surface.hex, tc.name, surfaceName, true, isTextCandidate(tc, surface.hex));
      // …and the surface's own colour as text ON this colour, which is only a
      // real pairing when the two sit on opposite sides of the light/dark line.
      if (isLightColor(tc.hex) !== surfaceIsLight) {
        add(surface.hex, tc.hex, surfaceName, tc.name, false, true);
      }
    }
  }

  // Keep the rows that mean something: a plausible text pairing, or any
  // failure. Now that failures are kept and up to three surfaces are tested, a
  // large palette produced ~324 rows and a 176KB config, a reference grid
  // nobody reads, which buries the handful of rows that matter.
  return pairs.filter(function (p) { return p.textPairing || !p.passes; });
}
