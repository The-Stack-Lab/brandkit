 ...
 *
 * Compared in oklch, not in sRGB bytes. A wide-gamut original and its sRGB
 * fallback share lightness and hue and differ in chroma, by as much as the
 * gamut demands, so a byte distance would reject exactly the values this
 * field exists to carry. Bytes also mislead near black, where 8/255 is a
 * visible step. The bounds cover hand-rounding to two decimals (Freeway and
 * the starter measure within 0.01 L and 2 degrees) and nothing like a real
 * disagreement such as a red hex beside a pink oklch.
 */
var SAME_L = 0.03;   // lightness, on oklch's 0 to 1 scale
var SAME_H = 10;     // hue, in degrees
var SAME_C = 0.03;   // how much LESS chromatic than the hex the original may be
var ACHROMATIC = 0.03; // below this chroma, hue carries no meaning
var BYTE_TOLERANCE = 4; // for an authored rgb()/hsl(), which is in gamut by definition

function oklchParts(str) {
  var m = /^oklch\(\s*([\d.]+)(%?)\s+([\d.]+)(%?)\s+([\d.]+)(?:deg)?\s*\)$/i.exec(String(str).trim());
  if (!m) return null;
  var L = parseFloat(m[1]) / (m[2] ? 100 : 1);
  var C = parseFloat(m[3]) * (m[4] ? 0.004 : 1); // 100% chroma is 0.4
  var H = parseFloat(m[5]) % 360;
  return (isNaN(L) || isNaN(C) || isNaN(H)) ? null : { L: L, C: C, H: H };
}

// Translucency in any syntax: `/ 0.5`, #rgba, #rrggbbaa, rgba(), hsla(), or a
// fourth comma argument. $value is opaque, so a translucent original is not it.
function hasAlpha(original) {
  var o = String(original).trim();
  if (/^#([0-9a-f]{4}|[0-9a-f]{8})$/i.test(o)) return true;
  if (o.indexOf('/') !== -1 || /^(rgba|hsla)\(/i.test(o)) return true;
  var args = /\(([^)]*)\)/.exec(o);
  return !!args && args[1].split(',').length === 4;
}

function isSameColor(wide, hex) {
  if (!wide || hasAlpha(wide.original)) return false;
  ...
  if (w && h) {
    ...
    if (w.C < h.C - SAME_C) return false;
    if (w.C < ACHROMATIC && h.C < ACHROMATIC) return true;
    var dh = Math.abs(w.H - h.H);
    return Math.min(dh, 360 - dh) <= SAME_H;
  }
  // Not oklch (or not parseable as plain oklch): an in-gamut color, so its
  // conversion must land on the hex within rounding.
  if (!HEX_RE.test(wide.hex) || !HEX_RE.test(hex) || wide.hex.length !== 7 || hex.length !== 7) return false;
  for (var i = 1; i < 7; i += 2) {
    if (Math.abs(parseInt(wide.hex.slice(i, i + 2), 16) - parseInt(hex.slice(i, i + 2), 16)) > BYTE_TOLERANCE) return false;
  }
  return true;
}
...
function buildTokensJson(config) {
  ...
  [['brand', 'brand'], ['neutrals', 'neutral'], ['semantic', 'semantic']].forEach(function (pair) {
    colorItems((config.colors || {})[pair[0]]).forEach(function (c) {
      ...
      if (!hasAlpha(parsed.original) && isSameColor(wide, parsed.hex) &&
