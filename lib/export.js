...
// Two 6-digit hex colors that differ only by rounding (see buildTokensJson).
var ROUNDING_TOLERANCE = 8;
function sameColorWithinRounding(hexA, hexB) {
  if (!HEX_RE.test(hexA) || !HEX_RE.test(hexB) || hexA.length !== 7 || hexB.length !== 7) return false;
  for (var i = 1; i < 7; i += 2) {
    if (Math.abs(parseInt(hexA.slice(i, i + 2), 16) - parseInt(hexB.slice(i, i + 2), 16)) > ROUNDING_TOLERANCE) return false;
  }
  return true;
}
...
function buildTokensJson(config) {
  ...
  [['brand', 'brand'], ['neutrals', 'neutral'], ['semantic', 'semantic']].forEach(function (pair) {
    colorItems((config.colors || {})[pair[0]]).forEach(function (c) {
      ...
      // oklch is only taken when it is the hex within rounding: hand-written
      // oklch values land up to 4 units per channel from their hex (measured
      // on Freeway and the starter), a real disagreement is far outside that.
      // An alpha hex wins over the oklch, which cannot carry the alpha.
      var wide = helpers.parseCssColor(filled(c.oklch));
      var hexHasAlpha = parsed.space === 'hex' && /^#([0-9a-f]{4}|[0-9a-f]{8})$/i.test(parsed.original);
      if (wide && !hexHasAlpha && sameColorWithinRounding(wide.hex, parsed.hex) &&
