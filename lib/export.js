...
function buildBrandMarkdown(config) {
  ...
  if (fonts.display || fonts.body) {
    ...
    if (fonts.display) {
      L.push('- **Display:** ' + fontLine(fonts.display));
      if (isUnset(fonts.display.description)) unfilled.push('Display font rationale');
    }
    if (fonts.body) {
      L.push('- **Body:** ' + fontLine(fonts.body));
      if (isUnset(fonts.body.description)) unfilled.push('Body font rationale');
    }
  }
