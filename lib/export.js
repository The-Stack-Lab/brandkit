...
function normalizeFormats(formats) {
  ...
  return canon.length ? canon : ['json', 'dtcg', 'md'];
}
...
function buildBrandMarkdown(config) {
  ...
  if (warns.length) {
    ...
    warns.forEach(function (a) {
      L.push('- ' + (a.fgName || a.fg) + ' on ' + (a.bgName || a.bg) + ' is ' + a.ratio + ' (' + a.rating + ') — large text only.');
    });
  }
...
// Drop null / empty-array / empty-object keys for a clean export.
function prune(obj) {
