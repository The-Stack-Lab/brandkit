...
var MAX_CHROMA = 0.5;  // beyond any display gamut: a typo such as `26` for `26%`
...
function isSameColor(wide, hex) {
  ...
  // Out-of-range numbers clip to a legal sRGB color and would pass the check
  // below, but the original is still nonsense to a consumer that reads it.
  var w = wide.space === 'oklch' ? oklchParts(wide.original) : null;
  if (w && (w.L > 1 || w.C > MAX_CHROMA)) return false;
  var a = channels(wide.hex), b = channels(hex);
  if (Math.abs(a[0] - b[0]) <= BYTE_TOLERANCE && Math.abs(a[1] - b[1]) <= BYTE_TOLERANCE &&
      Math.abs(a[2] - b[2]) <= BYTE_TOLERANCE) return true;

  if (wide.space !== 'oklch') return false;
  var onBoundary = b.some(function (v) { return v <= BYTE_TOLERANCE || v >= 255 - BYTE_TOLERANCE; });
  var h = oklchParts(helpers.hexToOklch(hex));
  if (!onBoundary || !w || !h) return false;
  if (h.C < ACHROMATIC || w.C <= h.C) return false;
  if (Math.abs(w.L - h.L) > SAME_L) return false;
  var dh = Math.abs(w.H - h.H);
