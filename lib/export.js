...
function buildBrandMarkdown(config) {
  ...
  if (fonts.display || fonts.body || scale.length) {
    ...
    if (scale.length) {
      ...
      // in caps, so a row that omits the flag misstates the style. Truthy like
      // the engine's specimen, except that an unfilled marker is not a
      // decision: projectTypeRow drops it from brand.json, so the brief must
      // not assert caps for the same row.
