...
// A type-scale row is copied whole. The exporter used to name the fields it
// kept, and every field it did not name vanished from brand.json: `uppercase`
// (set on Overline by brandkit's own scaffold) and `sample` first, and any
// field the schema gains later. A consumer reading "11px / 700 / 0.1em" with no
// caps flag renders a letterspaced sentence-case label, which is wrong, not
// merely incomplete. Only an unfilled marker is dropped, as everywhere else.
function projectTypeRow(t) {
  var out = {};
  Object.keys(t).forEach(function (k) {
    if (!isUnset(t[k])) out[k] = t[k];
  });
  return out;
}
...
/*
 * Is `wide` (what the author wrote in a palette color's `oklch` field) the same
 * color as `hex` (the sRGB value that becomes $value)? Only then may it be
 * exported as the authored form of that token.
 *
 * The two mistakes are not equal. Leaving `authored` off is harmless: it is
 * optional metadata and $value is still right. Attaching one that is a
 * different color makes a wide-gamut consumer paint the wrong thing. So this
 * is strict, and when in doubt it says no.
 *
 *   1. The original's sRGB conversion lands on the hex, within hand rounding
 *      (Freeway and the starter measure up to 4 per channel). No parsing, so
 *      every syntax the color parser accepts is covered.
 *   2. Otherwise, one narrow exception: a genuinely wide-gamut oklch beside its
 *      sRGB fallback. Conversion cannot confirm those (the original is outside
 *      sRGB), so it needs all of: a hex that sits on the gamut boundary, as a
 *      gamut-mapped fallback does; the same lightness and hue; and more chroma
 *      than the hex, within what oklch can express.
 */
var BYTE_TOLERANCE = 4;
var SAME_L = 0.03;     // lightness, on oklch's 0 to 1 scale
var SAME_H = 10;       // hue, in degrees
var MAX_CHROMA = 0.4;  // CSS's 100% chroma; beyond it is a typo such as `26` for `26%`
var ACHROMATIC = 0.03; // below this chroma a color has no usable hue

function channels(hex) {
  return [parseInt(hex.slice(1, 3), 16), parseInt(hex.slice(3, 5), 16), parseInt(hex.slice(5, 7), 16)];
}

function oklchParts(str) {
  var m = /^oklch\(\s*([\d.]+)(%?)\s+([\d.]+)(%?)\s+(-?[\d.]+)(?:deg)?\s*(?:\/\s*[\d.]+%?\s*)?\)$/i.exec(String(str).trim());
  if (!m) return null;
  var L = parseFloat(m[1]) / (m[2] ? 100 : 1);
  var C = parseFloat(m[3]) * (m[4] ? 0.004 : 1); // 100% chroma is 0.4
  var H = ((parseFloat(m[5]) % 360) + 360) % 360;
  return (isNaN(L) || isNaN(C) || isNaN(H)) ? null : { L: L, C: C, H: H };
}

// Is this original translucent? `/ 1`, `/ 100%` and a trailing `ff` are opaque.
// $value is always opaque, so a translucent original is not the same color.
function isTranslucent(original) {
  var o = String(original).trim();
  var hex = /^#(?:[0-9a-f]{3}([0-9a-f])|[0-9a-f]{6}([0-9a-f]{2}))$/i.exec(o);
  if (hex) return (hex[1] || hex[2]).toLowerCase().replace(/f/g, '') !== '';
  var alpha = /\/\s*([\d.]+)(%?)\s*\)$/.exec(o);
  if (!alpha) {
    var args = /\(([^)]*)\)/.exec(o);
    var parts = args ? args[1].split(',') : [];
    alpha = parts.length === 4 ? /^\s*([\d.]+)(%?)\s*$/.exec(parts[3]) : null;
    if (!alpha) return parts.length === 4; // a fourth argument we cannot read
  }
  var v = parseFloat(alpha[1]) / (alpha[2] ? 100 : 1);
  return isNaN(v) || v < 1;
}

function closeInOklch(p, q) {
  if (Math.abs(p.L - q.L) > SAME_L) return false;
  if (p.C < ACHROMATIC && q.C < ACHROMATIC) return true; // no usable hue on either side
  var dh = Math.abs(p.H - q.H);
  return Math.min(dh, 360 - dh) <= SAME_H;
}

function isSameColor(wide, hex) {
  if (!wide || isTranslucent(wide.original)) return false;
  if (!HEX_RE.test(wide.hex) || !HEX_RE.test(hex) || wide.hex.length !== 7 || hex.length !== 7) return false;
  var a = channels(wide.hex), b = channels(hex);
  var bytesMatch = Math.abs(a[0] - b[0]) <= BYTE_TOLERANCE && Math.abs(a[1] - b[1]) <= BYTE_TOLERANCE &&
    Math.abs(a[2] - b[2]) <= BYTE_TOLERANCE;
  if (wide.space !== 'oklch') return bytesMatch; // rgb(), hsl(), hex: in gamut by definition

  // An oklch we cannot read in full, or whose numbers are out of range (a
  // negative lightness, a chroma of 26 for 26%), is never vouched for. Both
  // clip to a legal sRGB color and would otherwise pass the byte check.
  var w = oklchParts(wide.original);
  if (!w || w.L > 1 || w.C > MAX_CHROMA) return false;

  // The byte check is only meaningful when the oklch is INSIDE sRGB. Outside
  // it, the parser clips each channel independently, which shifts hue and
  // lightness and can land exactly on an unrelated hex. Round-tripping tells
  // the two apart: an in-gamut color comes back as itself.
  var back = oklchParts(helpers.hexToOklch(wide.hex));
  var inGamut = !!back && closeInOklch(w, back) && Math.abs(w.C - back.C) <= ACHROMATIC;
  if (inGamut) return bytesMatch;

  // Wide gamut: compare with the hex itself, in oklch.
  var onBoundary = b.some(function (v) { return v <= BYTE_TOLERANCE || v >= 255 - BYTE_TOLERANCE; });
  var h = oklchParts(helpers.hexToOklch(hex));
  if (!onBoundary || !h || h.C < ACHROMATIC || w.C <= h.C) return false;
  return closeInOklch(w, h);
}
...
function buildBrandJson(config) {
  ...
  var out = {
    ...
    type: {
      ...
      scale: rows(config.typography).map(projectTypeRow)
    },
...
function buildTokensJson(config) {
  ...
  Object.keys(theme).forEach(function (key) {
    ...
    // --bk-* styles brandkit's own UI and is dropped from the rendered page;
    // shipping it here would hand a design tool a brandkit grey labelled as
    // this brand's token.
    if (require('./template').isChromeToken(key)) return;
  ...
  [['brand', 'brand'], ['neutrals', 'neutral'], ['semantic', 'semantic']].forEach(function (pair) {
    colorItems((config.colors || {})[pair[0]]).forEach(function (c) {
      ...
      //
      // `authored` must be the SAME color as $value, which is what the theme
      // convention guarantees. A palette entry has two fields that can
      // disagree (hex #FF0000 beside a pink oklch), and a wide-gamut consumer
      // would then paint a different color than the sRGB fallback. So the
      // oklch is taken only when isSameColor() says so. A translucent hex wins
      // (it is what the author wrote, as with theme tokens); a translucent
      // oklch beside an opaque hex is a disagreement and is left out.
      var wide = helpers.parseCssColor(filled(c.oklch));
      if (!isTranslucent(parsed.original) && isSameColor(wide, parsed.hex) &&
          wide.original.toUpperCase() !== parsed.hex) {
        ext.authored = wide.original;
        ext.colorSpace = wide.space;
      } else if (parsed.original.toUpperCase() !== parsed.hex) {
        ext.authored = parsed.original; // an alpha or 3-digit hex
...
function buildBrandMarkdown(config) {
  ...
  if (fonts.display || fonts.body || scale.length) {
    ...
    if (scale.length) {
      ...
      // Case is a column of its own: 0.1em tracking only reads correctly set
      // in caps, so a row that omits the flag misstates the style. This is the
      // same truthy test the engine's specimen applies, except that an unfilled
      // marker is not a decision: projectTypeRow drops it from brand.json and
      // the engine does not uppercase on it, so the brief must not say caps.
      var caps = function (t) { return (!isUnset(t.uppercase) && t.uppercase) ? 'caps' : ''; };
      L.push('| Style | Font | Size | Weight | Tracking | Leading | Case |');
      L.push('|---|---|---|---|---|---|---|');
      scale.forEach(function (t) {
        L.push('| ' + [t.name, t.font, t.size, t.weight, t.tracking, t.leading].map(cell).join(' | ') + ' | ' + caps(t) + ' |');
      });
    }
  }
