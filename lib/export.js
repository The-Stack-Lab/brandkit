...
function buildBrandMarkdown(config) {
  ...
  if (fonts.display || fonts.body || scale.length) {
    ...
    if (scale.length) {
      ...
      // in caps, so a row that omits the flag misstates the style. The engine
      // renders the specimen uppercase on the same truthiness test.
      L.push('| Style | Font | Size | Weight | Tracking | Leading | Case |');
      L.push('|---|---|---|---|---|---|---|');
      scale.forEach(function (t) {
        L.push('| ' + [t.name, t.font, t.size, t.weight, t.tracking, t.leading].map(cell).join(' | ') + ' | ' + (t.uppercase ? 'caps' : '') + ' |');
      });
    }
  }
