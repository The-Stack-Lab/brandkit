...
function projectBackground(config, b) {
  ...
  if (obj) {
    ...
    body.type = filled(obj.type) || (named ? 'color' : 'other');
    return body;
  }
...
function buildTokensJson(config) {
  ...
  rows(config.spacing).forEach(function (s) {
    if (!s || isUnset(s.token) || pxValue(s.px) === null) return;
...
function buildBrandMarkdown(config) {
  ...
  if (config.logos && config.logos.length) {
    L.push(''); L.push('## Logos');
    rows(config.logos).forEach(function (l) {
      if (isUnset(l.name)) return; // a nameless logo has nothing to point at
  ...
  if (config.changelog && config.changelog.length) {
    ...
    rows(config.changelog).forEach(function (e) {
